    ...
    !KF: modified: Only want scalar perturbations
    ...
    !KF: modified
    real(dl) kB, hbarc, meterstoMpC, Trad2, Trad_MpCinv, Tnu, Gnu_MpC, neutopacity
    ...
    !KF: Units of grho: Mpc**-2 (results.f90), so adotoa has units MpC**-1
    if (State%flat) then
        adotoa=sqrt(grho/3)
        cothxor=1._dl/tau
    else
    ...
    if (.not. EV%no_nu_multpoles) then
        ...
        !KF: This EV%no_nu_multpoles section is substantially modified
        clxrdot=-k*(4._dl/3._dl*z+qr)
        ayprime(EV%r_ix)=clxrdot
        qrdot=EV%denlk(1)*clxr-EV%denlk2(1)*pir
        ayprime(EV%r_ix+1)=qrdot

        !KF: Define Relevant Parameters and Convert to units of MpC, MpC**-1
        !KF: units: kB in eV/K, hbarc in eV m
        kB = 8.617333262145*(10._dl**(-5))
        hbarc = 1.9732698*(10._dl**(-7))
        !KF: units: MpC/m
        meterstoMpC = 3.24078*(10._dl**(-23))
        !KF: CMB temperature at given time (in K) and converted to units of MpC^-1
        Trad2 = State%CP%TCMB / a
        Trad_MpCinv = Trad2 * kB / hbarc / meterstoMpC
        !KF: Assume Neutrino Temperature is related to radiation temp by a constant, in units of MpC^-1
        Tnu = (4._dl/11._dl)**(1._dl/3._dl)*Trad_MpCinv
        !KF: Convert coupling from GeV^-2 to MpC^2
        Gnu_MpC = CP%Gnu*((10._dl**(-6))*hbarc*meterstoMpC)**2
        !KF: Compute (minus) the opacity of Neutrinos in MpC**-1
        neutopacity = a*(Gnu_MpC**2)*(Tnu**5)
        !KF: Assume this approximation is unchanged, only modified pirdot eqn
        if (EV%high_ktau_neutrino_approx) then
            ...
            pirdot= -3*pir*cothxor - clxrdot - neutopacity*pir
        ...
        !KF: Tight coupling approximation for neutrinos, following 1704.06657
        else if (neutopacity/adotoa > 60) then
            ix=EV%r_ix+2
            !Note: fits were run with these two lines instead, but doesn't change CAMB spectra
            !pirdot=(EV%denlk(2)*qr +8._dl/15._dl*k*sigma)/neutopacity
            !ayprime(ix)=pirdot
            pir=-(EV%denlk(2)*qr +8._dl/15._dl*k*sigma)/neutopacity
            ay(ix)=pir
            
        !KF: If no tight coupling, modified Boltzman eqns
        else
            ix=EV%r_ix+2
            if (EV%lmaxnr>2) then
                !pirdot=EV%denlk(2)*qr- EV%denlk2(2)*ay(ix+1)+8._dl/15._dl*k*sigma
                pirdot=EV%denlk(2)*qr- EV%denlk2(2)*ay(ix+1)+8._dl/15._dl*k*sigma- neutopacity*pir
                ayprime(ix)=pirdot
                do l=3,EV%lmaxnr-1
                    ix=ix+1
                    !ayprime(ix)=(EV%denlk(l)*ay(ix-1) - EV%denlk2(l)*ay(ix+1))
                    ayprime(ix)=(EV%denlk(l)*ay(ix-1) - EV%denlk2(l)*ay(ix+1)) - neutopacity*ay(ix)
                end do
                !  Truncate the neutrino expansion (unchanged)
                ix=ix+1
                ayprime(ix)=k*ay(ix-1)- (EV%lmaxnr+1)*cothxor*ay(ix)
            else
                pirdot=EV%denlk(2)*qr +8._dl/15._dl*k*sigma - neutopacity*pir
